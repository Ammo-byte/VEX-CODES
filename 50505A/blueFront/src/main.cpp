...
void pushOutForward() {
 pushOut.rotateTo(1280, deg, 50, rpm, true);
}
void pushOutReturn() {
 pushOut.rotateTo(0, rev, 80, rpm, false);
}
...
void outakeTime(int time) {
  intakeLeft.spinFor(time, msec, -50, velocityUnits::rpm);
  intakeRight.spinFor(time, msec, -50, velocityUnits::rpm);
}

void autonomous(void) {
  stopH();
  pushOut.rotateTo(275, deg, 60, rpm, true);
  pushOut.rotateTo(0, deg, 70, rpm, true);
  wait(50, msec);
  lift.rotateTo(250, deg, 50, rpm, true);
  wait(100, msec);
  lift.rotateTo(-55, deg, 70, rpm, false);
  ...
  wait(500, msec);
  intakeStop();
  leftFront.spin(fwd, 40, rpm);
  leftBack.spin(fwd, 40, rpm);
  rightFront.spin(fwd, 40, rpm);
  rightBack.spin(fwd, 40, rpm);
  wait(1250, msec);
  leftFront.stop();
  leftBack.stop();
  rightFront.stop();
...
int lifting() 
{
  if (Controller1.ButtonL1.pressing() && liftVal1==0 && Brain.timer(msec)>300)
  {
    Brain.resetTimer();
    liftVal1=1;
    lift.rotateTo(890,deg,100,velocityUnits::pct);
    lift.stop(hold);
  }
...
int removing() 
{
  ...
  if (Controller1.ButtonB.pressing() && liftVal2==0 && Brain.timer(msec)>300)
  {
    Brain.resetTimer();
    liftVal2=1;
    lift.rotateTo(565,deg,100,velocityUnits::pct);
    lift.stop(hold);
  }
...
int deposit() 
{
  if (Controller1.ButtonUp.pressing() && depositVal==0 && Brain.timer(msec)>300)
  {
    Brain.resetTimer();
    depositVal=1;
    pushOut.rotateTo(1300,deg,50,velocityUnits::pct);
    pushOut.stop(hold);
  }
