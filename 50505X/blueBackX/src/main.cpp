...
double mincreasing_speed (double starting_point, double current_position) {
    static const double acceleration_constant = 75.0;
...
void pushOutForward() {
 pushOut.rotateTo(980, deg, 51.2, rpm, true);
}
void pushOutReturn() {
...
void autonomous(void) {
  ...
  wait(50, msec);
  lift.rotateTo(260, deg, 100, rpm, true);
...
int deposit() {
  if (Controller1.ButtonUp.pressing() && depositVal==0 && Brain.timer(msec) > 300) {
    Brain.resetTimer();
    depositVal=1;
    pushOut.rotateTo(1120, deg, 55, velocityUnits::pct);
    pushOut.stop(hold);
  }
