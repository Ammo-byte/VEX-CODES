...
double mincreasing_speed (double starting_point, double current_position) {
    static const double acceleration_constant = 70.0;
...
double mdecreasing_speed (double ending_point, double current_position) {
    static const double deceleration_constant = 60.0;
...
void pushOutForward() {
  ...
  pushOut.rotateTo(1110, deg, 50, rpm, true);
}
...
void autonomous(void) {
  ...
  lift.rotateTo(-35, deg, 90, rpm, true);
  lift.resetRotation();
  wait(250, msec);
...
int depositAuto() {
  if (Controller1.ButtonUp.pressing() && depositVal==0 && Brain.timer(msec) > 300) {
    Brain.resetTimer();
    depositVal=1;
    pushOut.rotateTo(1140, deg, 50, velocityUnits::pct);
    pushOut.stop(hold);
  }
